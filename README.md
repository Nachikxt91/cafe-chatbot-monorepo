...
![System Architecture](./-CB 📦 Repository Structure
...
User sends message → Guard Agent validates request

Classification Agent determines intent (details/order/recommendation)

Appropriate specialized agent processes request

Order Taking Agent can trigger Recommendation Agent for upselling

Response returned to frontend with proper state management
...
Navigate to Space Settings → Variables
